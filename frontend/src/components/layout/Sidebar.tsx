...
const QuranSourcePreview: React.FC<{ source: QuranSource; surahs: Surah[] }> = ({
  source,
  surahs,
}) => {
  const [verseData, setVerseData] = useState<Verse | null>(null);
  ...
  const surahName =
    surahs.find((s) => s.nomor === source.surah)?.namaLatin || `Surah ${source.surah}`;
    ...
    <div>
      ...
      >
        <div className="flex items-center flex-grow min-w-0">
          {hasChildren ? (
            isNodeOpen ? (
              <ChevronDownIcon className="w-3.5 h-3.5 mr-2 flex-shrink-0 text-gray-400 dark:text-gray-500" />
            ) : (
              <ChevronRightIcon className="w-3.5 h-3.5 mr-2 flex-shrink-0 text-gray-400 dark:text-gray-500" />
            )
          ) : (
          ...
          {isInlineEditing ? (
            ...
            >
              <input
                ...
                className="flex-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 border-none text-xs p-1 rounded focus:ring-1 focus:ring-primary-500 outline-none h-6"
            ...
            <span
              className={`truncate ${hasChildren ? 'font-semibold text-gray-700 dark:text-gray-200' : 'text-gray-600 dark:text-gray-400'}`}
            >
              {item.title}
            </span>
          )}
      ...
      {isNodeOpen && (
        <div
          className="border-l border-gray-200 dark:border-gray-700 ml-[1.5rem]"
          style={{ marginLeft: `${level * 1.25 + 1.5}rem` }}
        >
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [editingItem, setEditingItem] = useState<
    (Partial<DirectoryItem> & { parentId?: string }) | null
  >(null);
  ...
  const sourcesMatch = (sources: Source[] | undefined, query: string): boolean => {
    ...
    return sources.some((source) => {
      if (source.type === 'quran') {
        return (
          source.arabic?.toLowerCase().includes(query) ||
      ...
      if (source.type === 'website') {
        return (
          source.title?.toLowerCase().includes(query) || source.url?.toLowerCase().includes(query)
        );
      }
      return false;
    });
          ...
          <nav className="p-4 space-y-2 flex-1">
            {isAdmin ? (
              <div className="space-y-1">
                <p className="px-4 text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">
                  Admin Control
                </p>
                <NavItem
                  icon={DashboardIcon}
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between px-2 mb-1">
                  <button
                    type="button"
                    onClick={() => setDirectoryOpen(!isDirectoryOpen)}
                    className="flex items-center flex-1 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-primary-50 dark:hover:bg-gray-700 p-2 rounded-lg"
                  >
                              ...
                              <div className="px-4 py-2 bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-primary-200 dark:border-primary-900/30 animate-fadeIn">
                                ...
                                >
                                  <input
                                    ...
                                    className="flex-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 border-none text-xs p-1.5 rounded focus:ring-1 focus:ring-primary-500 outline-none"
                                  />
                                  <button
                                    type="submit"
                                    className="p-1 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded"
            ...
            <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-3xl p-6 shadow-2xl animate-fadeIn max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold dark:text-white">
                  {editingItem.title && !editingItem.parentId
                    ? 'Edit Wawasan'
                    : 'Tambah Wawasan Baru'}
                </h3>
              ...
              {isLoadingSources ? (
                ...
                <div className="space-y-4">
                  ...
                  {/* Empty State */}
                  {!editingItem.title &&
                    (!editingItem.sources || editingItem.sources.every((s) => s.type === 'none')) &&
                    !editingItem.explanation && (
                ...
                <form onSubmit={saveItem} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-3.5 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 border border-gray-100 dark:border-gray-700 rounded-2xl focus:ring-2 focus:ring-primary-500 outline-none text-sm transition-all"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest ml-1">
                        Sumber Wawasan
                      </label>
                      <button
                        type="button"
