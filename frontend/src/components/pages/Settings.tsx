...
const Settings: React.FC<SettingsProps> = ({ toggleTheme, theme, setView, onLogout }) => {
  ...
  const faqData = [
    {
      q: 'Apa itu Manajemen Syariah OS?',
      a: 'SyariahOS adalah platform manajemen terpadu yang menyelaraskan standar operasional profesional dengan prinsip-prinsip syariah Islam.',
    },
    {
      q: 'Bagaimana cara kerja AI Generator?',
      a: 'AI kami menggunakan model Gemini 3 Pro untuk merumuskan langkah taktis berdasarkan visi Anda dalam kerangka POAC Islami.',
    },
    {
      q: 'Apakah data saya aman?',
      a: 'Seluruh data Anda disimpan secara lokal di browser Anda (Local Storage). Kami tidak menyimpan data sensitif di server eksternal tanpa izin Anda.',
    },
    {
      q: 'Bagaimana cara melakukan reset data?',
      a: "Anda dapat melakukan reset sistem melalui tab 'Aplikasi & Data' di menu pengaturan ini.",
    },
  ];
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fadeIn">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Kelola identitas, preferensi syariah, dan data aplikasi Anda.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md overflow-hidden flex flex-col md:flex-row min-h-[500px] border border-gray-100 dark:border-gray-700">
        ...
        <div className="w-full md:w-64 bg-gray-50 dark:bg-gray-900/50 border-r dark:border-gray-700 p-5 space-y-2">
          <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4 ml-2">
            Navigasi Panel
          </p>
          <button
            type="button"
        ...
        <div className="flex-1 p-8 md:p-10 overflow-y-auto">
          ...
          {activeTab === 'sharia' && (
            <div className="space-y-8 animate-fadeIn">
              <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 border-b dark:border-gray-700 pb-5">
                Konfigurasi Syariah
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setProfile({ ...profile, zakatRate: parseFloat(e.target.value) })
                    }
                    className="w-full px-5 py-3.5 bg-gray-50 dark:bg-gray-900 border border-transparent rounded-xl focus:ring-2 focus:ring-primary-500 text-gray-800 dark:text-gray-200 shadow-sm outline-none transition-all"
                ...
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4 ml-1">
                    Metode Kalender Operasional
                  </label>
                    ...
                    <label className="flex items-center cursor-pointer group">
                      <div className="relative">
                        <input
                          type="radio"
                          name="calc"
                          checked={profile.calculationMethod === 'Masehi'}
                          onChange={() => setProfile({ ...profile, calculationMethod: 'Masehi' })}
                          className="sr-only"
                        />
                        <div
                          className={`w-5 h-5 rounded-full border-2 transition-all ${profile.calculationMethod === 'Masehi' ? 'border-primary-600 bg-primary-600' : 'border-gray-300 dark:border-gray-600'}`}
                        >
                          {profile.calculationMethod === 'Masehi' && (
                            <CheckIcon className="w-4 h-4 text-white" />
                          )}
                        </div>
                      </div>
                      <span
                        className={`ml-3 text-sm font-bold transition-colors ${profile.calculationMethod === 'Masehi' ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}
                      >
                        Masehi (Umum)
                      </span>
                    </label>
                    <label className="flex items-center cursor-pointer group">
                      <div className="relative">
                        <input
                          type="radio"
                          name="calc"
                          checked={profile.calculationMethod === 'Hijri'}
                          onChange={() => setProfile({ ...profile, calculationMethod: 'Hijri' })}
                          className="sr-only"
                        />
                        <div
                          className={`w-5 h-5 rounded-full border-2 transition-all ${profile.calculationMethod === 'Hijri' ? 'border-primary-600 bg-primary-600' : 'border-gray-300 dark:border-gray-600'}`}
                        >
                          {profile.calculationMethod === 'Hijri' && (
                            <CheckIcon className="w-4 h-4 text-white" />
                          )}
                        </div>
                      </div>
                      <span
                        className={`ml-3 text-sm font-bold transition-colors ${profile.calculationMethod === 'Hijri' ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}
                      >
                        Hijriah (Syar'i)
                      </span>
                    </label>
          ...
          {activeTab === 'app' && (
            <div className="space-y-10 animate-fadeIn">
              <div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 border-b dark:border-gray-700 pb-5 mb-6">
                  Aplikasi & Tampilan
                </h3>
                <div className="flex items-center justify-between p-6 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Pilih antara tema terang atau gelap.
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    {theme === 'light' ? (
                      <MoonIcon className="w-7 h-7 text-indigo-600" />
                    ) : (
                  ...
                  >
                    <div className="text-left">
                      ...
                      <p className="text-xs text-primary-600 dark:text-primary-400 mt-0.5">
                        Unduh cadangan data Anda ke format JSON.
                      </p>
                    </div>
                  ...
                  >
                    <div className="text-left">
                      <p className="font-bold text-red-800 dark:text-red-300">
                        Reset Sistem Platform
                      </p>
                      <p className="text-xs text-red-600 dark:text-red-400 mt-0.5">
                        Hapus semua data muamalah secara permanen.
                      </p>
                    </div>
                    <div className="p-3 bg-white dark:bg-gray-800 rounded-xl shadow-sm group-hover:scale-110 transition-transform">
            ...
            <div className="space-y-8 animate-fadeIn pb-10">
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-8">
                  Temukan jawaban atas pertanyaan umum seputar platform SyariahOS.
                </p>
              </div>

              {/* FAQ Accordion */}
              <div className="space-y-3">
                <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1 mb-2">
                  FAQ - Pertanyaan Umum
                </p>
                {faqData.map((item, index) => (
                  <div
                    key={index}
                    className="border dark:border-gray-700 rounded-2xl overflow-hidden bg-gray-50/50 dark:bg-gray-900/30"
                  >
                    <button
                ...
                {pfpSourceType === 'url' ? (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-5 py-3.5 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 border border-transparent rounded-xl focus:ring-2 focus:ring-primary-500 outline-none text-sm transition-all shadow-inner"
                      placeholder="https://images.unsplash.com/..."
                    />
                    <p className="text-[10px] text-gray-400 mt-2 ml-1 italic">
                      Link langsung ke file JPG/PNG/WebP.
                    </p>
                  </div>
                ) : (
                  <div>
                    ...
                    >
                      ...
                      <span className="text-sm font-bold text-gray-500 group-hover:text-primary-600">
