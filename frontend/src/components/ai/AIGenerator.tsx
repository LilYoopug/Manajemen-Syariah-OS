...
const AIGenerator: React.FC = () => {
  ...
  const getPhaseIcon = (name: string) => {
    ...
    if (n.includes('takhthith') || n.includes('planning'))
      return <CalendarDaysIcon className="w-6 h-6" />;
    if (n.includes('tanzhim') || n.includes('organizing')) return <UsersIcon className="w-6 h-6" />;
    if (n.includes('tawjih') || n.includes('actuating'))
      return <PaperAirplaneIcon className="w-6 h-6" />;
    if (n.includes('riqabah') || n.includes('controlling'))
      return <DocumentCheckIcon className="w-6 h-6" />;
    ...
    <div className="space-y-6 animate-fadeIn">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Rancang roadmap keberkahan dengan kerangka POAC Islami dan kecerdasan AI.
        </p>
      </div>
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-md border border-gray-100 dark:border-gray-700 space-y-6">
          ...
          <div>
            <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3 ml-1">
              Prinsip Prioritas
            </label>
            <div className="grid grid-cols-1 gap-1.5">
              {syariahPrinciples.map((p) => (
                <label
                  key={p}
                  className="flex items-center p-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors group"
                >
                  <input
                    type="checkbox"
                    value={p}
                  ...
                  />
                  <span className="ml-3 text-xs font-medium text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">
                    {p}
                  </span>
                </label>
              ))}
          ...
          {!plan && !isLoading && (
            <div className="h-full flex flex-col items-center justify-center bg-white dark:bg-gray-800 rounded-2xl border-2 border-dashed border-gray-100 dark:border-gray-700 p-12 text-center shadow-md">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm leading-relaxed">
                Isi tujuan strategis Anda di samping dan biarkan AI merumuskan langkah taktis
                berbasis Syariah.
              </p>
            </div>
          )}
            ...
            <div className="space-y-6 animate-fadeIn pb-8">
              ...
              <div className="space-y-6">
                {plan.phases.map((phase, i) => (
                  <div
                    key={i}
                    className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md border border-gray-100 dark:border-gray-700 flex flex-col h-full hover:border-primary-200 dark:hover:border-primary-900 transition-all group"
                  >
                    <div className="flex items-center space-x-4 mb-4">
                    ...
                    <div className="space-y-2.5 mt-auto">
                      {phase.actions.map((action, ai) => (
                        <div
                          key={ai}
                          className="flex items-start bg-gray-50/50 dark:bg-gray-900/40 p-3 rounded-xl border border-transparent group-hover:border-gray-100 dark:group-hover:border-gray-700 transition-all"
                        >
                        ...
                        >
                          <div className="flex-1 min-w-0 mr-3">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span
                                className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full ${getPriorityColor(task.priority)}`}
                              >
                              ...
                              {task.resetCycle && task.resetCycle !== 'one-time' && (
                                <span className="text-[9px] text-primary-500 dark:text-primary-400 uppercase">
                                  ...
                                  {task.resetCycle === 'daily'
                                    ? 'Harian'
                                    : task.resetCycle === 'weekly'
                                      ? 'Mingguan'
                                      : task.resetCycle === 'monthly'
                                        ? 'Bulanan'
                                        : 'Tahunan'}
                                </span>
                              )}
                            ...
                            <p
                              className={`text-sm font-medium ${isAccepted ? 'text-gray-400 line-through' : 'text-gray-700 dark:text-gray-300'}`}
                            >
                              {task.title}
                            </p>
