...
const KpiCard: React.FC<KpiCardProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 lg:p-4 xl:p-6 rounded-2xl shadow-md flex items-center space-x-3 transition hover:shadow-lg hover:scale-105 min-w-0 overflow-hidden">
      <div
        className={`p-2.5 rounded-full flex-shrink-0 ${isIncrease ? 'bg-green-100 dark:bg-green-900/50' : 'bg-red-100 dark:bg-red-900/50'}`}
      >
        <Icon
          className={`w-5 h-5 lg:w-6 lg:h-6 ${isIncrease ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
        />
      </div>
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-baseline space-x-1 mt-0.5 flex-wrap">
          ...
          <span className={`text-[10px] lg:text-xs font-bold flex-shrink-0 ${changeColor}`}>
            {change}
          </span>
        </div>
      </div>
